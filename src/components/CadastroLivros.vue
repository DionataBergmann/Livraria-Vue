<template>
  <div class="container-fluid">
        <h1  id='title'>Livros
        <router-link to="/formlivros" class="btn btn-danger float-right ml-3">
        ...
        <div id='card-body'>
          <div class="card mb-4 shadow-lg" style="margin-left: 60px">
            ...
            <button
              ...
              title="Excluir"
              @click="excluir(livro.id, livro.titulo)"
            >
...
export default {
  ...
  methods: {
    ...
    excluir(id, titulo) {
      if (confirm(`Confirma exclusão do veículo '${titulo}'?`)) {
        axios.delete(this.$urlAPI + "/livros/" + id).then((response) => {
          if (response.status == 200) {
            alert(`Ok! Veículo '${titulo}' excluído com sucesso!`);
            this.listar();
          }
        });
      }
    ...
    pesquisar() {
      if (this.filtro.length == 0) {
        ...
        axios
          .get(this.$urlAPI + "/livros/" + this.filtro)
          .then((response) => (this.livros = response.data));
      }
    },
  },
